...
const Dashboard = ({ user, onLogout }) => {
  ...
  const [isPaused, setIsPaused] = useState(false); // Pause scroll on hover
    ...
    <div className="min-h-screen bg-slate-50 font-sans overflow-x-hidden">
      ...
      <main className="max-w-6xl mx-auto p-8">
        ...
        <div className="relative mb-8 transition-all duration-1000">
          ...
          {/* 1. WELCOME BANNER (Smaller Breadth: h-48 instead of h-64) */}
          <div 
            className={`transition-all duration-1000 ease-in-out overflow-hidden
              ${showWelcome ? 'opacity-100 max-h-96' : 'opacity-0 max-h-0'}`}
          >
            {/* FIX: Changed h-64 to h-48 (decreased size) */}
          ...
          {/* 2. ADVERTISEMENT SCROLLER (Larger Breadth: h-80 instead of h-64) */}
          <div 
          ...
          >
            {/* FIX: Changed h-64 to h-80 (increased size) */}
