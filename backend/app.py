...
def submit_claim(current_user):
    ...
    try:
        ...
        status_progression = [
            ...
            ('processing', 'Claim is being reviewed by our team'),
        ]
...
def login():
    ...
    try:
        ...
        if email == "admin@admin.com":
            ...
            # For demo purposes, always allow login if user exists
            # In production, you would check the password here

            # Always ensure user has an active policy (for both new and existing users)
            print(f"Checking policies for user {user.id} ({user.email})")
            active_policy = UserPolicy.query.filter_by(user_id=user.id, status='active').first()
            print(f"Active policy found: {active_policy}")

            if not active_policy:
                sample_policy = Policy.query.first()
                print(f"Sample policy found: {sample_policy}")
                if sample_policy:
                    ...
                    db.session.commit()
                    print(f"Created new policy for user {user.id}")
                else:
                    print("No sample policy found!")
...
def update_claim_status(current_user, id):
    ...
    if status not in ["approved", "rejected", "processing", "submitted", "under_review"]:
