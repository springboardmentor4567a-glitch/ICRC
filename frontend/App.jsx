...
function PolicyComparison({ onBack }) {
  ...
  const enrollPolicy = async (policyId) => {

  const userId = localStorage.getItem("user_id");

  try {
    const res = await fetch(`${API_URL}/user/policies/${policyId}`, {
      method: "POST",
      headers: {
        "user-id": userId
      }
    ...
    if (res.ok) {
      ...
      alert(data.detail || "Already enrolled");
    }
        ...
        {policies.map(p => {
          ...
          return (
            <div key={p.id} className="policy-card-ui">
              ...
              <button
  style={{ marginTop: "8px", background: "#27ae60", color: "white" }}
  onClick={() => enrollPolicy(p.id)}
>
  Enroll / Buy Policy
</button>
...
useEffect(() => {
  ...
  fetch(`${API_URL}/recommendations?ts=${Date.now()}`, {
    ...
    <div className="page-container">
      ...
      {list.map((r, i) => (
        <div className="card-box" key={i}>
          ...
          <p style={{ color: getRiskColor(r.score), fontWeight: "600" }}>
  Risk Level: {getRiskLevel(r.score)}
</p>
...
/* ================= DASHBOARD ================= */
function Dashboard({ userName, onLogout, setPage,  notificationCount  }) {
      ...
      <main className="grid">
        ...
        {!isAdmin && (
          <>
            ...
            <Card icon="🧾" title="File a Claim" onClick={() => setPage("claim")} />
            <Card icon="📍" title="Track Claims" onClick={() => setPage("claims")} />
        ...
        {isAdmin && (
          <>
            <Card icon="📋" title="All Claims (Admin)" onClick={() => setPage("adminClaims")} />
                <Card icon="📊" title="Analytics" onClick={() => setPage("adminAnalytics")} />
...
function Card({ icon, title, onClick, badge }) {
  return (
    <div className="card-box">
      <div className="icon">{icon}</div>
      <h3>
        {title} {badge > 0 && <span className="badge">({badge})</span>}
      </h3>
...
export default function App() {
  ...
  if (page === "notifications") return <Notifications onBack={() => setPage("dashboard")} />;
  ...
  if (page === "claim") return <ClaimForm onBack={() => setPage("dashboard")} />;
if (page === "claims") return <ClaimStatus onBack={() => setPage("dashboard")} />;
if (page === "adminClaims") return <AdminClaims onBack={() => setPage("dashboard")} />;
if (page === "adminAnalytics") return <AdminAnalytics onBack={() => setPage("dashboard")} />;
...
function Notifications({ onBack }) {
  ...
  return (
    <div className="page-container">
      ...
      {list.map(n => (
        <div className="card-box" key={n.id}>
          <p>{n.message}</p>
