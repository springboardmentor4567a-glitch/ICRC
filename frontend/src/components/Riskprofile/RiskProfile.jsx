...
function computeRiskLevel(data){
  try{
    ...
    if (age < 30) score += 1; else if (age < 50) score += 2; else score += 3;
    if (data.smoker) score += 3;
    if (data.health === 'poor') score += 3; else if (data.health === 'average') score += 1;
    const income = Number(data.income) || 0;
    if (income > 1000000) score -= 1; else if (income < 300000) score += 1;
    if (data.vehicle_ownership) score += 1;
    if ((data.dependents || 0) > 2) score += 1;
